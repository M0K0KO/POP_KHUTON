...
using System;
// using UnityEngine.PlayerLoop; // 불필요하여 제거

// 서버의 UserBase, UserResponse 모델에 해당 (id, nickname, level, exp)
[System.Serializable]
...
// 서버의 NewDetectedObjectInfo 모델에 해당
[System.Serializable]
...
// FastAPI 서버의 UserDataResponse 모델 (YOLO 서버의 /user_data/ 응답용)
[System.Serializable]
public class UserDataResponse // Combined data from main server's /user_data/
{
    public string id;
    public string nickname;
    public int? level; // 서버 응답에 따라 Nullable 처리
    public int? exp;   // 서버 응답에 따라 Nullable 처리
...
public class SSEObjectReceiver : MonoBehaviour
{
    ...
    public string mainServerBaseAddress = "http://localhost:8000"; // SSE 및 /user_data/ 엔드포인트 용 (YOLO 서버)
    public string authServerBaseAddress = "http://localhost:8001"; // 인증 관련 엔드포인트 용 (Auth 서버)
    ...
    public string sseTargetUserId = "defaultUser"; // SSE 연결 대상 사용자 ID
    ...
    public async Task<UserAuthResponseData> GetMyUserInfoAsync(string userIdToFetch)
    {
        ...
        // FastAPI /users/me/는 current_user_id를 Query Parameter로 받음
    ...
    // 사용자가 제공한 UpdateDetectionInfoInUnity 메서드 (Farm 관련 주석 유지)
